...
my $left;
my @array_left;
while ($left = $seqio_obj_R1 -> next_seq) {
	push @array_left, $left->seq;		 
}						    	
$left="@array_left";
...
my $right;
my @array_right;
while ($right = $seqio_obj_R2 -> next_seq) {
	push @array_right, $right;		 
}	
$right="@array_right";
...
my $leftTrimmed = $left ->get_all_clear_range(20);
my $rightTrimmed = $right ->get_all_clear_range(20);
